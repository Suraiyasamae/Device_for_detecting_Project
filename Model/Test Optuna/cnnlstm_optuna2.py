...
class TrainingVisualizer:
    def __init__(self):
        # หาพาทของโฟลเดอร์ Scripts
        scripts_dir = os.path.dirname(os.path.abspath(__file__))

        # สร้างโฟลเดอร์ result ภายใน Scripts
        self.save_dir = os.path.join(scripts_dir, 'result')

        # สร้างโฟลเดอร์หลักถ้ายังไม่มี
        if not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir)
            print(f"Created results directory at: {self.save_dir}")
        ...
        if not os.path.exists(self.trial_dir):
            ...
            print(f"Created trial plots directory at: {self.trial_dir}")
    ...
    def plot_trial_comparison(self, trial_results: List[Dict]):
        """Plot comparison of different trials"""
        if not trial_results:
            print("No trial results to plot")
            return

        plt.figure(figsize=(15, 10))

        # Extract data for plotting
        trial_nums = [result['trial_num'] for result in trial_results]
        best_val_accs = [result.get('best_val_acc', 0) for result in trial_results]
        final_train_accs = [result.get('final_train_acc', 0) for result in trial_results]
        final_val_accs = [result.get('final_val_acc', 0) for result in trial_results]

        # Plot accuracies
        x = range(len(trial_nums))
        width = 0.25

        plt.bar([i - width for i in x], final_train_accs, width, label='Final Training Accuracy', color='skyblue')
        plt.bar(x, final_val_accs, width, label='Final Validation Accuracy', color='lightgreen')
        plt.bar([i + width for i in x], best_val_accs, width, label='Best Validation Accuracy', color='salmon')

        plt.xlabel('Trial Number')
        plt.ylabel('Accuracy (%)')
        plt.title('Comparison of Trial Results')
        plt.xticks(x, [f'Trial {n}' for n in trial_nums])
        plt.legend()
        plt.grid(True, linestyle='--', alpha=0.7)

        # Add value labels on top of bars
        for i in x:
            plt.text(i - width, final_train_accs[i], f'{final_train_accs[i]:.1f}%',
                     ha='center', va='bottom')
            plt.text(i, final_val_accs[i], f'{final_val_accs[i]:.1f}%',
                     ha='center', va='bottom')
            plt.text(i + width, best_val_accs[i], f'{best_val_accs[i]:.1f}%',
                     ha='center', va='bottom')

        plt.tight_layout()
        save_path = os.path.join(self.save_dir, 'trial_comparison.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()

        print(f"Trial comparison plot saved to: {save_path}")
...
def train_model(trial, X_train, y_train, X_test, y_test, device, class_weights=None):
    ...
    activation = "ReLU"  # กำหนดค่าคงที่
    num_epochs = trial.suggest_int("epochs", 111, 120)
